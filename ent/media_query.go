...
func (_q *MediaQuery) QueryTweet() *TweetQuery {
...
func (_q *MediaQuery) WithTweet(opts ...func(*TweetQuery)) *MediaQuery {
	query := (&TweetClient{config: _q.config}).Query()
...
func (_q *MediaQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Media, error) {
	...
	if query := _q.withTweet; query != nil {
		if err := _q.loadTweet(ctx, query, nodes, nil,
			func(n *Media, e *Tweet) { n.Edges.Tweet = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *MediaQuery) loadTweet(ctx context.Context, query *TweetQuery, nodes []*Media, init func(*Media), assign func(*Media, *Tweet)) error {
...
func (_q *MediaQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withTweet != nil {
			_spec.Node.AddColumnOnce(media.FieldTweetID)
		}
	}
